...
public class AutoFinancedRiderTwo implements Job {
	...
	public void execute(JobExecutionContext context) {
		...
		try {
			...
			batchBean.setBatchJoId("autoFinanceBatchTwo");
			...
			+ " -parameters String:PaymentDate="+DateUtility.fifteenthDayOfMonthReduceMonthByOneString() + " -u"+ batchBean.getUsername() +" -p " + batchBean.getPassword() + " -jndi.url\r\n "
			+ batchBean.getJndiServer() + " -server.url " + batchBean.getServerUrl() + " -s ";
			BatchOperationCli batchOperationCli = new BatchOperationCli();
			batchOperationCli.startBatchCli(command, "autoFinanceBatchTwo");
			H2DatabaseLuncher.executeStatementInsertAndTruncate(
					"INSERT INTO BATCH_AUDIT (ID, BATCHTYPE_ID, LAST_RUN_DATE, NEXT_SCHDULE_DATE, BATCH_CSUCCESSFUL, BATCH_FAILED, BATCH_STARTTIME, BATCH_ENDTIME) VALUES (s.nextval,'"
							+ ScheduleConstantClass.autoFinanceBatchTwo + "','"
